...
def run_es(fitness_fn, dim, seed, sigma: float, n_evals: int):
    ...
    while total_evals < n_evals:
        ...
        # Paper notation: N(mu, sigma I) and text states sigma is the variance.
        # Therefore, per-dimension stddev is sqrt(sigma).
        std = float(np.sqrt(sigma))
        new_pop[1:] = mu + rng.normal(0.0, std, size=(n_pop - 1, dim))
    ...
    while total_evals < n_evals:
        ...
        for k in range(n_pop):
            if rng.random() < p_xo:
                ...
                alpha = rng.random()
                std = float(np.sqrt(sigma))
                eps = rng.normal(0.0, std, size=dim)
...
def _load_concrete(data_dir: Path) -> Tuple[np.ndarray, np.ndarray]:
...
def _load_energy(data_dir: Path) -> Tuple[np.ndarray, np.ndarray]:
